...
namespace NewWeb.Tests
{
    public abstract class NewWebTestBase : AbpIntegratedTestBase<NewWebTestModule>
    {
        ...
        protected void UsingDbContext(Action<NewWebDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<NewWebDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<NewWebDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<NewWebDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<NewWebDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<NewWebDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<NewWebDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<NewWebDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<NewWebDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<NewWebDbContext, Task<T>> func)
        {
            T result;
